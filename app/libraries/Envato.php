...
class Envato 
{
	function __construct()
	{
		...
		$this->callback = site_url('auth/oauth/envatocallback');
        $this->provider = new \Firstcoder\OAuth2\Client\Provider\Envato([
            'clientId' => $this->ci->config->item('envato_client_id'),
            'clientSecret' => $this->ci->config->item('envato_client_secret'),
	...
	function getUser()
	{
		...
		exit;
			...
			$this->ci->session->set_userdata('envato_access_token', serialize($token));
	...
	{
		if ($this->ci->session->userdata('envato_access_token')) {
			// Optional: Now you have a token you can look up a users profile data
			try {
				$token = unserialize($this->ci->session->userdata('envato_access_token'));
