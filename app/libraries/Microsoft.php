...
class Microsoft 
{
	function __construct()
	{
		...
		$this->callback = site_url('auth/oauth/mscallback');
        $this->provider = new Stevenmaguire\OAuth2\Client\Provider\Microsoft([
            'clientId' => $this->ci->config->item('microsoft_client_id'),
            'clientSecret' => $this->ci->config->item('microsoft_client_secret'),
	...
	{
		if (isset($_GET['error'])) {
			$this->ci->session->unset_userdata('microsoft_access_token');
		...
		$options = [
			...
			'scope' => ['wl.basic', 'wl.signin', 'wl.work_profile', 'wl.emails'] // array or string
		];
		...
		// Check given state against previously stored one to mitigate CSRF attack
			...
			$this->ci->session->set_userdata('microsoft_access_token', serialize($token));
	...
	{
		if ($this->ci->session->userdata('microsoft_access_token')) {
			// Optional: Now you have a token you can look up a users profile data
			try {
				$token = unserialize($this->ci->session->userdata('microsoft_access_token'));
