...
class Linkedin 
{
	function __construct()
	{
		...
		$this->callback = site_url('auth/oauth2/lncallback');
        $this->provider = new League\OAuth2\Client\Provider\LinkedIn([
            'clientId' => $this->ci->config->item('linkedin_client_id'),
            'clientSecret' => $this->ci->config->item('linkedin_client_secret'),
	...
	function getUser()
	{
		...
		// Check given state against previously stored one to mitigate CSRF attack
			...
			$this->ci->session->set_userdata('linkedin_access_token', serialize($token));
	...
	{
		if ($this->ci->session->userdata('linkedin_access_token')) {
			
			// Optional: Now you have a token you can look up a users profile data
			try {
				$token = unserialize($this->ci->session->userdata('linkedin_access_token'));
