...
class Bitbucket 
{
	function __construct()
	{
		...
		$this->callback = site_url('auth/oauth/bbcallback');
        $this->provider = new Stevenmaguire\OAuth2\Client\Provider\Bitbucket([
            'clientId' => $this->ci->config->item('bitbucket_key'),
            'clientSecret' => $this->ci->config->item('bitbucket_secret'),
	...
	function getUser()
	{
		...
		// Check given state against previously stored one to mitigate CSRF attack
			...
			$this->ci->session->set_userdata('bitbucket_access_token', serialize($token));
	...
	{
		if ($this->ci->session->userdata('bitbucket_access_token')) {

			// Optional: Now you have a token you can look up a users profile data
			try {
				$token = unserialize($this->ci->session->userdata('bitbucket_access_token'));
