...
class Instagram 
{
	function __construct()
	{
		...
		$this->callback = site_url('auth/oauth2/instacallback');
        $this->provider = new League\OAuth2\Client\Provider\Instagram([
            'clientId' => $this->ci->config->item('instagram_client_id'),
            'clientSecret' => $this->ci->config->item('instagram_client_secret'),
	...
	function getUser()
	{
		...
		// Check given state against previously stored one to mitigate CSRF attack
			...
			$this->ci->session->set_userdata('instagram_access_token', serialize($token));
	...
	{
		if ($this->ci->session->userdata('instagram_access_token')) {
			
			// Optional: Now you have a token you can look up a users profile data
			try {
				$token = unserialize($this->ci->session->userdata('instagram_access_token'));
