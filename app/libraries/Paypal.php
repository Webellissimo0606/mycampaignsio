...
class Paypal 
{
	function __construct()
	{
		...
		$this->callback = site_url('auth/oauth/paypalcallback');
        $this->provider = new \Firstcoder\OAuth2\Client\Provider\Paypal([
            'clientId' => $this->ci->config->item('paypal_client_id'),
            'clientSecret' => $this->ci->config->item('paypal_client_secret'),
	...
	function getUser()
	{
		...
		$options = [
			...
			'scope' => ['openid profile email address phone'] // array or string
		];
		...
		// Check given state against previously stored one to mitigate CSRF attack
			...
			$this->ci->session->set_userdata('paypal_access_token', serialize($token));
	...
	{
		if ($this->ci->session->userdata('paypal_access_token')) {

			// Optional: Now you have a token you can look up a users profile data
			try {
				$token = unserialize($this->ci->session->userdata('paypal_access_token'));
