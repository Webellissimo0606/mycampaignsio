...
class Analytics extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->simple_api_key = $simple_api_key;
    ...
    public function index() {
        ...
        elseif ( $this->ci_auth->is_logged_in( false ) ) {
            ...
            if ( isset( $_GET['code'] ) ) {
                ...
                $authenticate_code = $this->client->authenticate( $_GET['code'] );

                if( ! $authenticate_code || empty( $authenticate_code ) ){
                    echo 'Authorization failed';
                }
                else{

                    $auth_data['user_id'] = $user_id;
                    $auth_data['access_token'] = $authenticate_code['access_token'];
                    $auth_data['token_type'] = $authenticate_code['token_type'];
                    $auth_data['expires_in'] = $authenticate_code['expires_in'];
                    $auth_data['refresh_token'] = $authenticate_code['refresh_token'];
                    $auth_data['created'] = $authenticate_code['created'];
                    
                    $this->analytics_model->insertUserAuthData( $auth_data );

                    // TODO: Do they really need?
                    // $_SESSION['token'] = $authenticate_code;
                    // $this->client->setAccessToken( $_SESSION['token'] );

                    
                    // Update list of user's accounts.
                    $this->getUserGAAccounts( true, true );
    ...
    public function getGoogleAnalytics() {
        ...
        elseif ( $this->ci_auth->is_logged_in(false) ) {
            ...
            $profiles = $analytics->management_profiles->listManagementProfiles( $domainDetails['ga_account'], '~all' );
...
//        return $accounts;
    ...
    public function getUserGAAccounts_OLDER() {
    ...
    public function getUserGAAccounts( $update_accounts_list = false, $return_value = false ) {

        $client = $this->client;
        
        $user_id = $this->ci_auth->is_logged_in() && ! $this->ci_auth->is_logged_in(false) ? $this->ci_auth->get_user_id() : $this->input->post('userId');

        $force_update_accounts = $this->input->post('force_account_updtes');
        $force_update_accounts = $force_update_accounts ? $force_update_accounts : $update_accounts_list;
        ...
        if( $user_id ){
            ...
            if( $force_update_accounts ){

                if ( ! $client->getAccessToken() ) { $this->getUserAuthenticated( $user_id ); }
                ...
                $query = $this->db->get_where('user_ga_account', array('user_id' => $user_id));
                $results = $query->result();

                if( $results ){
                    foreach ($results as $row) {
                        $accounts[] = array( $row->id => $row->name );
                    }
                }
            }
        ...
        elseif( ! $this->ci_auth->is_logged_in() ) {
            redirect(site_url('auth/login'));
            exit;
        }
        elseif( $this->ci_auth->is_logged_in(false) ) {
                ...
                foreach ($query->rows as $val) {
                    if (!empty($saved_keywords)) {
                        if (!(in_array($val->keys[0], $saved_keywords))) {
                            ...
                            $keywords[] = array('<input type="checkbox" name="add_keyword[]" value="' . $val->keys[0] . '" class="add_keyword">',$val->keys[0], $val->clicks, $val->impressions);
                        }
                    } else {
                        $keyword['keyword'] = $val->keys[0];
                        $keyword['clicks'] = $val->clicks;
