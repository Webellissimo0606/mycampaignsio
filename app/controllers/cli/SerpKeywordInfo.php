<?php
class SerpKeywordInfo extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('serp/serpkeywordinfo_model');
    }
    public function index()
    {
        ...
        foreach ($allDomains as $domains) {
            ...
            foreach ($searchengines as $engines) {
                $exist = $this->serpkeywordinfo_model->checkKeywordInfoExist(date('Y-m-d'), $engines['id']);
                if (!$exist) {
                    ...
                    //storing data
                    $this->serpkeywordinfo_model->insertSerpKeywordInfo($domains['id'], $engines['id'], $result['result'], $result['status']);
                    sleep(2);
                }
            }
        }
